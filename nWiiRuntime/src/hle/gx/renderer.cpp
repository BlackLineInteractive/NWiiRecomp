...
static std::map<uint32_t, Texture2D> s_texture_cache;

// Texture decoding helper
static void LoadTextureIfNecessary(uint32_t base_addr, uint32_t width, uint32_t height, uint8_t format) {
    if (base_addr == 0 || width == 0 || height == 0) return;
    if (s_texture_cache.find(base_addr) != s_texture_cache.end()) return; // Already loaded
    ...
    if (format == 0x0) { // GX_TF_I4 (8x8 blocks, 32 bytes)
                ...
                for (int y = 0; y < 8; ++y) {
                    for (int x = 0; x < 8; ++x) {
                        ...
                        if (px < width && py < height) {
                            uint8_t val = ::nwii::runtime::g_mmu->read8(block_addr + (y * 8 + x) / 2);
                            ...
                            pixels[out_idx + 1] = i;
                            pixels[out_idx + 2] = i;
                            pixels[out_idx + 3] = i; // GameCube intensity usually populates alpha as well, or at least intensity
                        }
                    }
                }
                addr += 32;
            ...
            for (uint32_t bx = 0; bx < width; bx += 8) {
                ...
                for (int y = 0; y < 4; ++y) {
                    for (int x = 0; x < 8; ++x) {
                        ...
                        uint32_t py = by + y;
                        if (px < width && py < height) {
                            uint8_t i = ::nwii::runtime::g_mmu->read8(block_addr + (y * 8 + x));
            ...
            for (uint32_t bx = 0; bx < width; bx += 8) {
                ...
                for (int y = 0; y < 4; ++y) {
                    for (int x = 0; x < 8; ++x) {
                        ...
                        uint32_t py = by + y;
                        if (px < width && py < height) {
                            uint8_t val = ::nwii::runtime::g_mmu->read8(block_addr + (y * 8 + x));
                            uint8_t a = val >> 4; a = (a << 4) | a;
                            uint8_t i = val & 0xF; i = (i << 4) | i;
            ...
            for (uint32_t bx = 0; bx < width; bx += 4) {
                ...
                for (int y = 0; y < 4; ++y) {
                    for (int x = 0; x < 4; ++x) {
                        ...
                        uint32_t py = by + y;
                        if (px < width && py < height) {
                            uint16_t val = ::nwii::runtime::g_mmu->read16(block_addr + (y * 4 + x) * 2);
                            uint8_t a = val >> 8;
                            uint8_t i = val & 0xFF;
            ...
            for (uint32_t bx = 0; bx < width; bx += 4) {
                ...
                for (int y = 0; y < 4; ++y) {
                    for (int x = 0; x < 4; ++x) {
                        ...
                        uint32_t py = by + y;
                        if (px < width && py < height) {
                            uint8_t a = nwii::runtime::g_mmu->read8(block_addr + (y * 4 + x) * 2 + 0);
                            uint8_t r = nwii::runtime::g_mmu->read8(block_addr + (y * 4 + x) * 2 + 1);
                            uint8_t g = nwii::runtime::g_mmu->read8(block_addr + 32 + (y * 4 + x) * 2 + 0);
                            uint8_t b = nwii::runtime::g_mmu->read8(block_addr + 32 + (y * 4 + x) * 2 + 1);
        ...
        for (uint32_t by = 0; by < height; by += 8) {
            for (uint32_t bx = 0; bx < width; bx += 8) {
                for (int sub = 0; sub < 4; ++sub) {
                    ...
                    if (c0 > c1) {
                        for (int i=0; i<3; ++i) pal[2][i] = (2 * pal[0][i] + pal[1][i]) / 3;
                        for (int i=0; i<3; ++i) pal[3][i] = (pal[0][i] + 2 * pal[1][i]) / 3;
                        pal[2][3] = 255; pal[3][3] = 255;
                    } else {
                        for (int i=0; i<3; ++i) pal[2][i] = (pal[0][i] + pal[1][i]) / 2;
                        pal[2][3] = 255;
                    ...
                    for (int y = 0; y < 4; ++y) {
                        for (int x = 0; x < 4; ++x) {
                            ...
                            if (px < width && py < height) {
                                ...
                                // The highest 2 bits of the 32-bit word correspond to the first pixel.
        ...
        for (size_t i = 0; i < pixels.size(); i += 4) {
            pixels[i+0] = 255; pixels[i+1] = 0; pixels[i+2] = 0; pixels[i+3] = 128;
        }
    }
    ...
    printf("Loaded texture format %d from 0x%08X (%dx%d) as TexID %d\n", format, base_addr, width, height, tex.id);
}
    ...
    for (const auto& cmd : commands) {
        if (cmd.type == GXCommandType::XFRegister) {
            ...
            if (use_tex0 && g_state.texStages[0].base_addr != 0) {
                LoadTextureIfNecessary(g_state.texStages[0].base_addr, g_state.texStages[0].width, g_state.texStages[0].height, g_state.texStages[0].format);
